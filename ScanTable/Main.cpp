...
int main()
{
  ...
  t1.Insert(e2); // поместим в таблицу е2
	...
	finding = t1.Find(3); // найдем элемент со значением 3
	finding = t1.Find(6); // попробуем найти элемент со значением 6
	deleting = t1.Delete(2) // удалим элемент со значением 2
	return 0;
}
